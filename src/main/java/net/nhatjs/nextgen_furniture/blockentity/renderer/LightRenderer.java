...
public class LightRenderer implements BlockEntityRenderer<LightBlockEntity> {
    ...
    public LightRenderer(BlockEntityRendererProvider.Context ctx) {
        led = mc.getModelManager().getModel(ModelResourceLocation.inventory(NhatJSNextGenFurnitureModClient.LIGHT_MODERN_EXTRA));
    }
    ...
    public void render(LightBlockEntity entity, float tickDelta, PoseStack matrices,
        ...
        matrices.translate(-0.5, -0.5, -0.5);
        bmr.renderModel(matrices.last(), vc, state, led, 1, 1, 1, light, overlay);
        if (powered) {
            bmr.renderModel(matrices.last(), vc, state, led, 1, 1, 1, LightTexture.FULL_BRIGHT, overlay);
        }
        matrices.popPose();
    }
}
